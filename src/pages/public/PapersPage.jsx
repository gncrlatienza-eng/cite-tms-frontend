  ...
  return (
    <>
      <style>{`
        ...
        .sp-bm-btn.saved:hover { color: #7f1d1d; }
        .sp-bm-btn:disabled { opacity: 0.4; cursor: default; }
        .sp-bm-icon { transition: fill 0.15s; }
        ...
        .sp-btn-pdf {
          display: inline-flex; align-items: center; gap: 5px;
          padding: 6px 14px; border-radius: 7px;
          border: 1.5px solid #9b0000; background: #9b0000;
        ...
        .sp-empty-title {
          font-family: 'DM Serif Display', serif;
          font-size: 20px; color: #111827; margin-bottom: 8px;
        }
        .sp-empty-sub {
            ...
            <div className="sp-sidebar-body">
              ...
              <div className="sp-sb-group">
                ...
                {PROGRAM_FILTERS.map(({ key, label }) => (
                  <button
                    key={key} type="button"
                    className={`sp-sb-btn${program === key ? " active" : ""}`}
            ...
            {!loading && !error && displayed.length === 0 && (
              <div className="sp-empty">
                ...
                <p className="sp-empty-sub">
                  {papers.length === 0
                    ? "Papers will appear here once they've been added by an administrator."
                    : `No papers match "${query}". Try a different keyword or adjust your filters.`}
                </p>
                {query && (
                  <button className="sp-empty-clear" onClick={() => setQuery("")}>
                    Clear search
                  </button>
                )}
              </div>
            ...
            {!loading && !error && displayed.map((paper, i) => {
              ...
              return (
                <article className="sp-card" key={paper.id} style={{ animationDelay: `${i * 0.03}s` }}>
                  ...
                  {/* Abstract */}
                  {paper.abstract && (
                    <p className="sp-snippet">{paper.abstract}</p>
                  )}
