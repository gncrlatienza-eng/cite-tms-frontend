...
export default function TermsPage() {
  ...
  const handleAccept = async () => {
    ...
    setBusy(true); setErr("");
    try {
      const { error } = await supabase.from("users").update({ has_accepted_terms: true }).eq("id", user.id);
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tp-page { height: 100vh; overflow: hidden; display: flex; flex-direction: column; background: #fafafa; font-family: 'DM Sans', system-ui, sans-serif; }

        .tp-header { background: #fff; border-bottom: 1px solid #efefef; padding: 16px 40px; display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; }
        .tp-header-title { font-family: 'Schibsted Grotesk', serif; font-size: 18px; color: #111827; display: flex; align-items: center; gap: 10px; }
        .tp-back-link { display: flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 600; color: #9b0000; text-decoration: none; }
        .tp-back-link:hover { color: #7f1d1d; }

        .tp-body { flex: 1; display: flex; flex-direction: column; min-height: 0; max-width: 660px; width: 100%; margin: 0 auto; padding: 24px 40px; }
        .tp-card { flex: 1; display: flex; flex-direction: column; min-height: 0; background: #fff; border: 1px solid #f0f0f0; border-radius: 16px; overflow: hidden; box-shadow: 0 2px 12px rgba(0,0,0,0.04); }
        .tp-card-header { padding: 22px 24px 16px; border-bottom: 1px solid #f3f4f6; flex-shrink: 0; }
        .tp-card-title { font-family: 'Schibsted Grotesk', serif; font-size: 21px; color: #111827; margin-bottom: 4px; }
        .tp-card-sub { font-size: 13px; color: #6b7280; line-height: 1.5; }

        .tp-scroll-hint { margin: 12px 24px 0; padding: 9px 13px; border-radius: 8px; font-size: 12px; display: flex; align-items: center; gap: 7px; flex-shrink: 0; }
        ...
        .tp-content { overflow-y: auto; padding: 20px 24px; flex: 1; min-height: 0; }
        ...
        .tp-footer { padding: 16px 24px 22px; border-top: 1px solid #f3f4f6; flex-shrink: 0; }
        ...
        .tp-readonly-note { font-size: 13px; color: #6b7280; text-align: center; padding: 16px 0 4px; flex-shrink: 0; }
        ...
        <div className="tp-body">
          <div className="tp-card">
            <div className="tp-card-header">
              <div className="tp-card-title">Submission Terms & Conditions</div>
              <p className="tp-card-sub">
                DLSL Thesis Management System — {isNewUser ? "please read and accept to continue." : "last updated 2025."}
              </p>
            </div>
            ...
            {isNewUser && (
              <div className={`tp-scroll-hint ${scrolled ? "done" : "pending"}`}>
                {scrolled ? (
                  <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>You've read all the terms — check the box below to continue.</>
                ) : (
                  <><Clock size={13} />Scroll to the bottom to read all terms before continuing.</>
                )}
              </div>
            )}
            ...
            {isNewUser ? (
              <div className="tp-footer">
                ...
                <div className="tp-check-row">
                  <input type="checkbox" id="tp-check" checked={checked} disabled={!scrolled || busy} onChange={(e) => setChecked(e.target.checked)} />
                ...
                <div className="tp-actions">
                  <button className="tp-btn-accept" disabled={!checked || busy} onClick={handleAccept}>
                    {busy ? "Saving…" : "I Agree — Continue"}
                  </button>
                </div>
              </div>
            ) : (
              <div className="tp-readonly-note">Questions? Contact the CITE-TMS administrator.</div>
            )}
          </div>
        </div>
      </div>
