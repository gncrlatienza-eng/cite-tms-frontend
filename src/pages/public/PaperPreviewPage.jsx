    ...
    <div className="pp-modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="pp-modal" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="pp-modal-actions">
          <button className="pp-modal-btn-cancel" onClick={onClose} disabled={submitting}>
            Cancel
          </button>
          <button
    ...
    const load = async () => {
      ...
      try {
        const { data, error: err } = await supabase
          .from("papers")
          .select("id, title, authors, year, course_or_program, abstract, file_path, access_type")
          .eq("id", id)
          .single();
        if (err) throw err;
    ...
    <>
      <style>{`
        ...
        .pp-breadcrumb-link {
          ...
          color: #9b0000; font-size: 13px; font-family: inherit;
          font-weight: 500; transition: color 0.15s; display: flex; align-items: center; gap: 5px;
        }
        ...
        .pp-year-pill {
          background: #fef2f2; color: #9b0000;
        ...
        .pp-btn-download:hover { opacity: 0.92; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(22,101,52,0.35); }
        .pp-btn-download:active { transform: translateY(0); }
        ...
        .pp-btn-request:active { transform: translateY(0); }
        ...
        .pp-skel {
          ...
          animation: ppShimmer 1.4s infinite linear;
        }
        ...
        .pp-modal {
          ...
          overflow: hidden;
          animation: ppModalIn 0.2s cubic-bezier(0.34,1.4,0.64,1);
        }
        @keyframes ppModalIn {
        ...
        .pp-modal-title {
          font-family: 'DM Serif Display', serif;
          font-size: 20px; font-weight: 400; color: #111827; margin-bottom: 4px;
        }
        .pp-modal-sub {
        ...
        .pp-modal-textarea {
          ...
          color: #111827; resize: vertical; min-height: 110px;
        ...
        .pp-modal-char {
          font-size: 11.5px; color: #d1d5db; margin-left: auto;
        }
        ...
        .pp-spinner {
          ...
          animation: ppSpin 0.75s linear infinite; flex-shrink: 0;
        }
          ...
          <div className="pp-skel-page">
            <div className="pp-layout">
              <div>
                ...
                <div className="pp-skel" style={{ height: 14, width: "92%", marginBottom: 8 }} />
        ...
        {!loading && !error && paper && (
          <div className="pp-layout">
            ...
            <div className="pp-sidebar-card">
              ...
              {/* Restricted — logged in, no request yet */}
              {!isOpen && user && !requestStatus && (
                <>
              ...
              {/* Restricted — pending */}
              {!isOpen && user && requestStatus === "pending" && (
                <>
              ...
              {/* Restricted — approved */}
              {!isOpen && user && requestStatus === "approved" && paper.publicUrl && (
                <>
              ...
              {/* Restricted — rejected */}
              {!isOpen && user && requestStatus === "rejected" && (
                <>
              ...
              <div className="pp-meta-list">
                {paper.year && (
                  <div className="pp-meta-row">
                    <span className="pp-meta-key">Year</span>
                    <span className="pp-meta-val">{paper.year}</span>
                  </div>
                )}
                {paper.course_or_program && (
                  <div className="pp-meta-row">
                    <span className="pp-meta-key">Program</span>
                    <span className="pp-meta-val">{paper.course_or_program}</span>
                  </div>
                )}
                {paper.authors?.length > 0 && (
                  <div className="pp-meta-row">
                    <span className="pp-meta-key">Author{paper.authors.length > 1 ? "s" : ""}</span>
                    <span className="pp-meta-val">{paper.authors.join(", ")}</span>
                  </div>
                )}
                <div className="pp-meta-row">
                  <span className="pp-meta-key">Access</span>
                  <span className="pp-meta-val">{isOpen ? "Open Access" : "Restricted"}</span>
                </div>
              </div>
            </div>
          </div>
